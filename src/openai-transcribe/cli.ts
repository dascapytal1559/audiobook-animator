...
import { getGeminiSummary, findChapterAudioFile } from "../gemini-summary/paths";
import { 
  getChapterAudioPath, 
 ...
 */
async function splitAudioIntoChunks(audioPath: string, totalDuration: number): Promise<string[]> {
  console.log(`Splitting audio file into ${Math.ceil(totalDuration / MAX_CHUNK_DURATION)} chunks of max ${MAX_CHUNK_DURATION} seconds...`);
  ...
  for (let i = 0; i < chunkCount; i++) {
    ...
    await execAsync(`ffmpeg -y -i "${audioPath}" -ss ${start} -t ${duration} -acodec copy "${chunkPath}"`);
 ...
 */
function stitchTranscriptions(transcripts: Transcript[], chunkDurations: number[]): Transcript {
  let combinedText = '';
  ...
  transcripts.forEach((transcript, index) => {
    ...
    const timeOffset = chunkDurations.slice(0, index).reduce((sum, d) => sum + d, 0);
...
// --- Core Logic ---
async function transcribeAudio(audioPath: string, expectedDuration: number, chapterSummary?: string): Promise<OpenAI.Audio.TranscriptionVerbose> {
  validateAudioFile(audioPath);
  const stats = fs.statSync(audioPath);
  console.log(`File size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
  console.log("Starting transcription...");

  console.log(`Expected audio duration: ${expectedDuration} seconds (${Math.floor(expectedDuration/60)}:${(expectedDuration%60).toString().padStart(2, '0')})`);
...
async function transcribeChapter(chapterDir: string, forceChunks: boolean = false): Promise<void> {
  ...
  console.log(`Total duration: ${totalDuration} seconds (${Math.floor(totalDuration/3600)}:${Math.floor((totalDuration%3600)/60).toString().padStart(2, '0')}:${Math.floor(totalDuration%60).toString().padStart(2, '0')})`);
  ...
  if (!needsChunking) {
    ...
    const response = await transcribeAudio(audioPath, totalDuration, shortSummary);
    ...
    for (let i = 0; i < chunkPaths.length; i++) {
      ...
      const chunkDurationPath = path.join(chapterDir, `chunk_${i}.duration.json`);
      const durationData = JSON.parse(fs.readFileSync(chunkDurationPath, 'utf8'));
      ...
      const rawResponsePath = path.join(chapterDir, `chunk_${i}.transcript.res.json`);
      const transcriptPath = path.join(chapterDir, `chunk_${i}.transcript.json`);
      
      // Get transcription from OpenAI - use summary for every chunk for consistent context
      const response = await transcribeAudio(chunkPath, chunkDuration, shortSummary);
    ...
    const stitchedTranscript = stitchTranscriptions(chunkTranscripts, chunkDurations);
...
async function main() {
  ...
  try {
    ...
    program
      .description("Transcribe chapter audio files with automatic chunking for files >45 minutes")
